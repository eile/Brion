...
const std::string mappingAttributes[] =
                        { "type", "sections", "soma", "axon", "basal", "apic" };
const std::string dataAttributes[] =
                          { "rank", "tstart", "tstop", "Dt", "dunit", "tunit" };
    ...
    , _path( initData.getURI().getPath() )
{
...

    {
        ...
        if( !file.getNumObjs( ))
            LBTHROW( std::runtime_error( "File is empty: " +
                                         file.getFileName( )));
        ...
        try
        {
            ...
            dataset.openAttribute( dataAttributes[1] ).read(
                                     H5::PredType::NATIVE_DOUBLE, &_startTime );
            dataset.openAttribute( dataAttributes[2] ).read(
                                     H5::PredType::NATIVE_DOUBLE, &_endTime );
            dataset.openAttribute( dataAttributes[3] ).read(
                                     H5::PredType::NATIVE_DOUBLE, &_timestep );
            _dunit = "mV";
            _tunit = "ms";
        }
        catch( ... )
        {
            LBTHROW( std::runtime_error( _path.string() + " not a valid H5"\
...
floatsPtr CompartmentReportHDF5::loadFrame( const float timestamp ) const
{
    ...
    for( GIDSetCIter cellID = _gids.begin(); cellID != _gids.end(); ++cellID )
    {
        ...
        targetSpace.selectHyperslab( H5S_SELECT_SET, readCounts,
                                     targetOffsets );
...
void CompartmentReportHDF5::updateMapping( const GIDSet& gids )
{
         ...
         ++cellID, ++cellIndex )
    {
        ...
        const H5::H5File& file = _file.getId() ? _file :
                                             _files.find( *cellID )->second;

        std::stringstream cellName;
        cellName << "a" << *cellID;
        const std::string datasetName = "/" + cellName.str() + "/" +
                                        _reportName + "/" + mappingDatasetName;
        H5::DataSet dataset;
        H5E_BEGIN_TRY
    ...
    for( offsetsIt = _offsets.begin(),
         countsIt = _counts.begin();
         offsetsIt != _offsets.end();
         ++offsetsIt, ++countsIt )
    {
        ...
        {
            countsIt->push_back( calcCompartmentCounts( offsetsIt,
                                                        sectionOffsetsIt,
                                                        _offsets,
...
void CompartmentReportHDF5::_openFile( const uint32_t cellID )
{
    ...
    {
        LBTHROW(
            std::runtime_error( "ReportReaderHDF5: error opening file:"
                             + filename.string( )));
    }
    _files[cellID] = file;
}
